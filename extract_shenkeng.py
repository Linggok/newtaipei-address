# -*- coding: utf-8 -*-
"""從新北市門牌位置數值資料中篩選深坑區資料（代號65000180），填入行政區、去重並重新排序"""
import csv

input_path = r'c:\Users\user\Desktop\新北市\新北市門牌位置數值資料.csv'
output_path = r'c:\Users\user\Desktop\newtaipei\深坑區門牌位置數值資料.csv'

# 深坑區代碼
SHENKENG_AREACODE = '65000180'
...
with open(input_path, 'r', encoding='utf-8') as infile:
    ...
    for row in reader:
        # 檢查第二欄（索引1）是否為深坑區代碼
        if len(row) >= 2 and row[1] == SHENKENG_AREACODE:
            # 提取欄位：areacode(1), 路名(4), 巷(6), 弄(7)，去除前後空白
            areacode = row[1] if len(row) > 1 else ''
            road = (row[4] if len(row) > 4 else '').strip()
            lane = (row[6] if len(row) > 6 else '').strip()
            alley = (row[7] if len(row) > 7 else '').strip()

            # 建立輸出列：areacode, 行政區, 路名, 巷, 弄
            output_row = [areacode, '深坑區', road, lane, alley]
