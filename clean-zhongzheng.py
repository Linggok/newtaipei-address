...
# 要刪除：省市縣市代碼(0), 鄉鎮市區代碼(1), 村里(3), 鄰(4), 橫座標(10), 縱座標(11)
# 要保留：區名(2), 街路段(5), 地區(6), 巷(7), 弄(8), 號(9)
...
if idx_road is None and len(headers) >= 6:
    idx_road = 5
if idx_area is None and len(headers) >= 7:
    idx_area = 6
...
for i, line in enumerate(lines[1:], 1):
    ...
    area = (cols[idx_area] if idx_area is not None and idx_area < len(cols) else '').strip()
    ...
    # 注意：我們不保留「號」欄位，因為輸出格式只需要 areacode,行政區,路名,巷,弄
    output_row = ['', district, road, lane, alley]
