...
output_path = os.path.join(base_dir, '鶯歌區門牌位置數值資料.csv')
    ...
    for encoding in encodings:
        try:
            with open(input_path, 'r', encoding=encoding) as infile:
                ...
                for row in reader:
                    # 檢查第二欄（索引1）是否為鶯歌區代碼
                    if len(row) >= 2 and row[1] == YINGGE_AREACODE:
                        ...
                        output_row = [areacode, '鶯歌區', road, lane, alley]
